...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name_raw) || empty($email_raw) || empty($password)) {
        ...
        mysqli_stmt_bind_param($stmt_check, 's', $email_raw);
        ...
        if ($existing) {
            ...
            $stmt = mysqli_prepare($conn,
                "INSERT INTO users (name, email, password, role, created_at) VALUES (?, ?, ?, 'siswa', NOW())"
            );
            mysqli_stmt_bind_param($stmt, 'sss', $name_raw, $email_raw, $hashed);
            $ok = mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
            if ($ok) {
                $success = 'Akun berhasil dibuat! Silakan <a href="login.php" style="color:#155724;font-weight:700;">login di sini</a>.';
            } else {
...
<head>
    ...
    <style>
        ...
        .auth-box { position:relative; background:#fff; padding:36px 28px; border-radius:14px; box-shadow:0 8px 24px rgba(0,0,0,.15); width:100%; max-width:380px; animation:fadeUp .5s ease; }
...
<div class="auth-box">
    ...
    <?php endif; ?>
    <div class="auth-links">Sudah punya akun? <a href="login.php">Login</a></div>
</div>
</body>
</html>